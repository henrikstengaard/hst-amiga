...
namespace Hst.Amiga.DataTypes.UaeFsDbs
{
    public static class UaeFsDbWriter
    {
        ...
        public static byte[] Build(UaeFsDbNode node)
        {
            ...
            var amigaName = node.AmigaName ?? string.Empty;
            var normalName = node.NormalName ?? string.Empty;

            nodeBytes[0] = node.Valid;
            BigEndianConverter.ConvertUInt32ToBytes(node.Mode, nodeBytes, 0x1);
            TextHelper.WriteNullTerminatedString(Encoding.ASCII, node.AmigaNameUnicode ?? amigaName, nodeBytes, 0x5, 257);
            TextHelper.WriteNullTerminatedString(Encoding.ASCII, node.NormalNameUnicode ?? normalName, nodeBytes, 0x106, 257);
            ...
            TextHelper.WriteNullTerminatedString(Encoding.Unicode, node.AmigaNameUnicode ?? amigaName, nodeBytes, 0x25c, 514);
            TextHelper.WriteNullTerminatedString(Encoding.Unicode, node.NormalNameUnicode ?? normalName, nodeBytes, 0x45e, 514);
