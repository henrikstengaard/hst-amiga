namespace Hst.Amiga.FileSystems.FastFileSystem
{
    ...
    public static class DirBlockWriter
    {
        public static async Task<byte[]> BuildBlock(DirBlock dirBlock, uint blockSize)
        {
            ...
            await blockStream.WriteBigEndianInt32(dirBlock.Type);
            await blockStream.WriteBigEndianInt32(dirBlock.HeaderKey);
            await blockStream.WriteBigEndianInt32(dirBlock.HighSeq);
            await blockStream.WriteBigEndianInt32(dirBlock.HashTableSize);
            await blockStream.WriteBigEndianInt32(0); // r1
            await blockStream.WriteBigEndianUInt32(0); // checksum

            for (var i = 0; i < Constants.HT_SIZE; i++)
            {
                await blockStream.WriteBigEndianInt32(dirBlock.HashTable[i]);
            }
            
            // await blockStream.WriteLittleEndianInt32(0); // r2
            // await blockStream.WriteLittleEndianInt32(0); // r2
            blockStream.Seek(4 * 2, SeekOrigin.Current);
            await blockStream.WriteBigEndianInt32(dirBlock.Access);
            ...
            blockStream.Seek(4, SeekOrigin.Current);
            await blockStream.WriteBigEndianInt32(dirBlock.RealEntry);
            await blockStream.WriteBigEndianInt32(dirBlock.NextLink);
            ...
            await blockStream.WriteBigEndianInt32(dirBlock.NextSameHash);
            await blockStream.WriteBigEndianInt32(dirBlock.Parent);
            await blockStream.WriteBigEndianInt32(dirBlock.Extension);
            await blockStream.WriteBigEndianInt32(dirBlock.SecType);
